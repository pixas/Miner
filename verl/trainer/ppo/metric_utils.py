...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # group by batch.non_tensor_batch['uid']
    uid2scores = defaultdict(list)
    for i, uid in enumerate(batch.non_tensor_batch.get("uid", [str(i) for i in range(len(sequence_score))])):
        if non_aborted_mask[i]:
            uid2scores[uid].append(sequence_score[i].item())
    
    all_correct_ratio = np.mean([1.0 if all(s > 0 for s in scores) else 0.0 for scores in uid2scores.values()])
    at_least_one_correct_ratio = np.mean([1.0 if any(s > 0 for s in scores) else 0.0 for scores in uid2scores.values()])
    ...
    metrics = {
        ...
        "response/all_correct_ratio": all_correct_ratio,
        "response/at_least_one_correct_ratio": at_least_one_correct_ratio,
        # reward
