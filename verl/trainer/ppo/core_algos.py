...
def compute_srpo_advantage(
    ...
    outcome_adv, _ = compute_grpo_vectorized_outcome_advantage(token_level_rewards, response_mask, index, epsilon, norm_adv_by_std_in_grpo, config)
...
def compute_grpo_ent_advantage(
    ...
    sequence_entropy = (entropy * response_mask).sum(dim=-1) / response_mask.sum(dim=-1) # [B, ]
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            if acc > 0 and scores[i] < 0:
                scores[i] = 0.0
            elif acc < 0 and scores[i] > 0:
                scores[i] = 0.0
            else:
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                if id2std[idx] > epsilon:
                    ...
                    correct_index = torch.where(scores_tensor == torch.max(scores_tensor))[0]
                    incorrect_index = torch.where(scores_tensor != torch.max(scores_tensor))[0]
                    id2correct_index[idx] = correct_index
                    id2sim[idx] = sim_matrix
                    
                    incorrect_sim = sim_matrix[incorrect_index, :][:, correct_index] # [|incorrect_index|, |correct_index|]
                    incorrect_sim_mean = torch.mean(incorrect_sim, dim=1)  # [|incorrect_index|]
                    ...
                    if incorrect_index.shape[0] == 1:
                        group_weights = torch.tensor([1.0], device=scores.device, dtype=scores.dtype)
                    else:
                        if config.neg_scale.scale_mode == 'exp':
                            ...
                            group_weights = torch.exp(-config.neg_scale.scale * normalized_incorrect_sim_mean)
                        elif config.neg_scale.scale_mode == 'shift':
                            softmax_incorrect_sim_mean = torch.softmax(incorrect_sim_mean / config.neg_scale.scale, dim=0)
                            ...
                            signs = torch.where(sim_ranks >= (incorrect_index.shape[0] - 1) / 2, -1.0, 1.0)
                    ...
                    for i, idx_within_group in enumerate(incorrect_index):
                        ...
                        if config.neg_scale.as_reward:
                            scores[batch_idx] = scores[batch_idx] + config.neg_scale.scale * group_weights[i]
                            pass
