...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        from .dp_actor import DataParallelActor
        ...
        if self._is_ref:
            ...
            self.ref_policy = DataParallelActor(config=self.config.ref, actor_module=self.ref_module_fsdp)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        compute_emb = data.meta_info.pop("compute_emb", True)
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, hidden_states, token_logits = self.actor.compute_log_prob(data=data, calculate_entropy=True, calculate_emb=compute_emb)
            if hidden_states is not None:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "response_emb": hidden_states, "token_logits": token_logits},
                    meta_info={"temperature": self.config.rollout.temperature},
                )
            else:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "token_logits": token_logits},
                    meta_info={"temperature": self.config.rollout.temperature},
                )
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_compute_response_embed")
    def compute_response_emb(self, data: DataProto):
        # when is_lora is True, we use the actor without lora applied to calculate the log_prob
        # which is mostly used for ref log_prob calculation
        assert self._is_actor
        if self._is_offload_param:
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output = self.actor.compute_output_emb(data=data)
                # output, entropys = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            output = DataProto.from_dict(
                tensors={"response_emb": output},
                meta_info={"temperature": self.config.rollout.temperature},
            )
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_response_emb", logger=logger)
    ...
    def update_sampling_params(self, **kwargs):
        self.rollout.update_sampling_params(**kwargs)
        pass
